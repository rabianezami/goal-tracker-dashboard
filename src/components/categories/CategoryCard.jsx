...
export default function CategoryCard({ title, total, active, completed }) {
  ...
  return (
    ...
    >
      ...
      >
        <Typography>{t("list.total", { total: total })}</Typography>
        <Typography>{t("list.active", { active: active })}</Typography>
        <Typography>{t("list.completed", { completed: completed })}</Typography>
      </Box>
      <Box sx={{ width: "100%" }}>
        <LinearProgress
          variant="determinate"
